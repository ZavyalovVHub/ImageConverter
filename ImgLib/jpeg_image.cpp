...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    * Эта процедура заполняет содержимое jerr и возвращает адрес jerr
    ...
    */
    ...
    /* Назначаем пункт назначения для данных */
    ...
    */
    ...
    std::vector<JSAMPLE> buffer(row_stride); /* временный буфер */

    /* Шаг 5: while (остаются строки изображения) */
    /*           jpeg_write_scanlines(...); */
    while (cinfo.next_scanline < cinfo.image_height) {
        /* jpeg_write_scanlines ожидает массив указателей на scanlines.
        * В данном случае длина массива составляет всего один элемент, но вы могли бы передавать
        * более одной scanlines за раз, если это удобнее.
        */
        ...
        // используем один указатель на буффер, чтобы не плодить много буфферов
        row_pointer[0] = buffer.data();
        (void) jpeg_write_scanlines(&cinfo, row_pointer, 1);
    }

    /* Шаг 6: Окончание сжатия */

    jpeg_finish_compress(&cinfo);
    /* После окончание сжатия мы можем закрыть выходной файл. */
    fclose(outfile);
